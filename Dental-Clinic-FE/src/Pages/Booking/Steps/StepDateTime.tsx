...
export default function StepDateTime({ data, updateData, onNext, onPrev }: StepProps) {
  ...
  const fetchSlots = async () => {
    ...
    try {
      // 1. Lấy token từ localStorage (Key này phải khớp với lúc bạn lưu khi login)
      ...
      if (err.response && err.response.status === 401) {
         ...
         setError("Không thể tải lịch làm việc. Vui lòng kiểm tra kết nối.");
      }
    } finally {
      setLoading(false);
    }
       ...
       <div className="mt-6">
            {loading ? (
                <div className="text-center py-12">
                    ...
                    <p className="text-gray-500">Đang kiểm tra lịch trống...</p>
                </div>
            ) : error ? (
                ...
                <div className="text-center py-12 bg-gray-50 rounded-xl border border-dashed border-gray-300">
                    <p className="text-gray-500 font-medium">Bác sĩ không có lịch làm việc vào ngày này.</p>
